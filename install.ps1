...
$tmp = Join-Path ([System.IO.Path]::GetTempPath()) "rano_install"
...
# Find binary
$bin = Get-ChildItem -Path $extractDir -Recurse -Filter "rano.exe" | Select-Object -First 1
if (-not $bin) {
...
Copy-Item $bin.FullName (Join-Path $Dest "rano.exe") -Force
Write-Ok "Installed to $Dest\rano.exe"
...
if (-not $path.Contains($Dest)) {
  if ($EasyMode) {
    ...
    Write-Warn "Add $Dest to PATH to use rano"
  }
}
...
if ($Verify) {
  ...
  & "$Dest\rano.exe" --version | Out-Null
  Write-Ok "Self-test complete"
}

Write-Ok "Done. Binary at: $Dest\rano.exe"
Write-Host ""
Write-Info "Try it:"
Write-Host "  $Dest\rano.exe --pattern claude --pattern codex --pattern gemini --stats-interval-ms 5000"
